...
def main():
    ...
    parser.add_argument("--output_name", type=str, default="superdiff_grid.png")
    parser.add_argument("--batch_size", type=int, default=1, help="Number of samples (rows) to generate per lift")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--steps", type=int, default=700)

    # Grid Arguments
    parser.add_argument("--lift_min", type=float, default=-1.0, help="Minimum lift value")
    parser.add_argument("--lift_max", type=float, default=1.0, help="Maximum lift value")
    parser.add_argument("--lift_step", type=float, default=0.25, help="Step size for lift")
    ...
    # Define lift values including the max value
    # using np.arange usually excludes stop, so we add a tiny epsilon or use num points if preferred.
    # Here we stick to a while loop logic or inclusive linspace to be safe
    epsilon = 1e-5
    lift_values = np.arange(args.lift_min, args.lift_max + args.lift_step - epsilon, args.lift_step)

    # Ensure -1 and 1 are exactly hit if in range
    lift_values = [round(x, 3) for x in lift_values]
    print(f"Generating grid for Lift Values: {lift_values}")
    ...
    for lift_val in lift_values:
        print(f"Sampling for Lift = {lift_val} ...")

        # RESET RNG for every lift value so we see the *same* noise evolution
        rng = jax.random.PRNGKey(args.seed)

        images, _ = superdiff_and_sampler(
            rng=rng,
        ...
        # Store as numpy
        all_lift_images.append(np.array(images))

    # --- 4. Save Results as Grid ---
    from torchvision.utils import save_image
    import torch

    # all_lift_images is a list of N_lifts arrays, each shape [B, H, W, C]
    # We want to arrange them such that:
    # Row 1: Seed 1 (Lift -1), Seed 1 (Lift -0.75) ...
    # Row 2: Seed 2 (Lift -1), Seed 2 (Lift -0.75) ...

    # 1. Stack to shape [N_lifts, B, H, W, C]
    stack_np = np.stack(all_lift_images, axis=0)

    # 2. Transpose to [B, N_lifts, H, W, C] (Group by Seed)
    stack_np = stack_np.transpose(1, 0, 2, 3, 4)

    # 3. Reshape to flat list [B * N_lifts, H, W, C]
    B, N, H, W, C = stack_np.shape
    flat_np = stack_np.reshape(B * N, H, W, C)

    # 4. Convert to Torch [Total, C, H, W]
    if flat_np.shape[-1] == 1 or flat_np.shape[-1] == 3:
        images_torch = torch.from_numpy(flat_np).permute(0, 3, 1, 2)
    else:
        images_torch = torch.from_numpy(flat_np)

    out_path = os.path.join(args.run_dir_1, "final_samples", args.output_name)
    os.makedirs(os.path.dirname(out_path), exist_ok=True)

    # nrow = number of columns = number of lift values
    save_image(images_torch, out_path, nrow=len(lift_values), padding=2, normalize=True)
    print(f"Saved grid samples to: {out_path}")
    print(f"Grid Layout: {args.batch_size} rows (seeds) x {len(lift_values)} columns (lifts)")
