#!/usr/bin/env bash
#
# Launches a DIAGNOSTIC training job for the autoencoder on a single PNEUMONIA sample.
#
...
export WANDB_TAGS="ae:pneumonia:diagnostic"

# --- Enable overfitting on one sample ---
export OVERFIT_ONE=1
export OVERFIT_K=0

export RUN_NAME="ae_pneumonia_diagnostic_$(date +%Y%m%d)"

echo "Submitting SLURM job for AE DIAGNOSTIC training on PNEUMONIA data..."
echo "------------------------------------------------"
