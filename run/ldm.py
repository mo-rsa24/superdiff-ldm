...
def load_autoencoder_(config_path, ckpt_path):
    ...
    if isinstance(restored, (list, tuple)) and restored:
        ...
        gen_state = restored
    if isinstance(gen_state, dict) and "params" in gen_state:
        gen_params = gen_state["params"]
    else:
        gen_params = gen_state
    ae_params = gen_params.get("ae", gen_params)
    print("Autoencoder loaded successfully.")
...
def main():
    ...
    if args.overfit_one:
        ...
        loader_kwargs = dict(batch_size=batch_size, shuffle=True, num_workers=0, drop_last=True, pin_memory=True)
