...
\begin{abstract}
Compositional image generation is achieved either through natural-language
conjunction---a monolithic prompt such as ``a cat and a dog''---or through
score-level logical operators such as SuperDiff AND\@.
These paradigms are widely treated as interchangeable, yet they encode
fundamentally different inductive biases: the former delegates composition
to the model's learned co-occurrence statistics, while the latter enforces
joint probability mass directly in the score field, independent of
corpus priors.
We provide the first systematic, trajectory-level evidence that both
operators occupy geometrically distinct regions of the denoising flow
manifold, diverging from a shared noise origin within 2--4 denoising
steps and terminating 247--407 latent units apart across all concept pairs
tested.
To quantify the expressibility boundary between paradigms, we train a
closed-loop conditioning inverter $f_\theta$ that maps SuperDiff-AND
outputs to the nearest achievable SD3.5 conditioning vector $p^*$, and
measure the residual composability gap at three levels: CLIP cosine
similarity and LPIPS at image level, MSE in VAE latent space, and
step-wise trajectory divergence along matched denoising paths.
This residual is predictable from the CLIP cosine similarity between the
individual concept embeddings, yielding an operational taxonomy of pair
composability that partitions concepts by how much of their logical
conjunction is semantically articulable.
Guided velocity interpolation at $\alpha\!=\!0.3$ reduces the
trajectory gap by 41--68\% relative to pure SuperDiff without collapsing
semantic coherence, establishing hybrid composition as a practical bridge
